...
export default class Game extends Component {
  displayName = Game.name
	...
	{
		super(props);
		this.state={data:[], pos: 1, loading: true ,wuerfelNumber: 1, selectFigureID: 'none'};
		...
		this.setzeFigur = this.setzeFigur.bind(this);
		this.getData();
	}
	...
	selectFigure(event)
	{		
		this.setState({selectFigureID: event.target.id});
	...
	setzeFigur()
	{
		//send wuerfelNumber, klicked Position and currentUser to server
		fetch('http://localhost:8080/figurSetzen?gewuerfelteZahl=' + this.state.wuerfelNumber + '&indexMomentanerSpieler=0&position=' + this.state.selectFigureID)
      .then(response => response.json())
      .then(data => {
	...
	<div id='wholeBody' >
		...
		<div id='wuefelBox'>
				...
				<p id='wuerfelText' onclick={this.wuerfeln} >{this.state.wuerfelNumber}</p>
			</div>
			
			<div id='selectedFigureBox'>
				<p>selectedFigure: {this.state.selectFigureID}</p>				
				<button onClick={this.setzeFigur}>setze Figur</button>
			</div>
		<div id='board' onClick={this.selectFigure}>
			<table>
				<tr>
				  <td id='a1Start'>a1</td>
				  <td id='a2Start'>a2</td>
